  ...
  reducers: {
    ...
    getLocationSuccess: (state, {payload}) => {
      ...
      const newData = [...data].map((d, i) => {
        const cio = checkInsideOne(payload.lat, payload.lon, d['Latitude'], d['Longitude'], d['Radius'])
        const newObject = Object.assign({}, d)
        const angle = CalculateAngle(payload.lat, payload.lon, d['Latitude'], d['Longitude'])
        ...
        closest = (i === 0) ? newObject : (newObject.distance < closest.distance) ? newObject : closest

        return newObject
      })
      setTimeout(() => window.dispatchEvent(new Event(EVENTS.GPS_SUCCESS)), 20)
      return {...state, isError: false, location: payload, data: newData, closest, isInsideOfAnyStores }
    },
    ...
    getTotalDistance:  (state, action) =>{
      ...
      const addedDist = Number(state.totalDist) + Number(action.payload)
      console.log(addedDist)
      return {...state, totalDist : (addedDist).toFixed(1)}
    }
  }
})
